...
export function useChatSessionOperations() {
  ...
  // セッション切替時の初期化
  const initializeSession = useCallback(async (sessionId: string | null) => {
    const state = useChatSessionStore.getState();
  ...
  // LLMプロバイダー更新
  const updateLLMProvider = useCallback(async (sessionId: string | null, provider: LLMProvider) => {
    const state = useChatSessionStore.getState();
  ...
  const sendMessage = useCallback(
    async (sessionId: string | null, content: string) => {
      ...
      try {
        ...
        // メッセージの受信が完了してから永続化
        // 最終的なストア上のメッセージ
        const messageToSave = useChatSessionStore.getState().activeMessages.find((m) => m.id === streamingMessageId);
        if (messageToSave) {
