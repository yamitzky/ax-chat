    ...
    this.version(2)
      ...
      .upgrade(async (tx) => {
        ...
        if (oldSessions.length > 0) {
          const newSessions = oldSessions.map(
            (oldSession: Record<string, unknown>) => {
              const oldMetadata =
                (oldSession.metadata as Record<string, unknown>) || {}
              return {
                id: oldSession.id,
                ...
                data: {
                  ...
                  llmProvider:
                    (oldMetadata.provider as string) || "gemini-flash",
                  useWebSearch:
                    oldMetadata.useWebSearch !== undefined
                      ? (oldMetadata.useWebSearch as boolean)
                      : true,
                },
                metadata: {
